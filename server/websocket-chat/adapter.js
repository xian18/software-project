                        ...
                        shimPeerConnection: function () {
                                    ...
                                    this.iceOptions.iceServers = iceServers.filter(function (server) {
                                        if (server && server.urls) {
                                            ...
                                            urls = urls.filter(function (url) {
                                                return (
                                                    url.indexOf('turn:') === 0 && url.indexOf('transport=udp') !== -1
                                                );
                                            })[0];
                                            return !!urls;
                                        }
                                ...
                                this._localIceCandidatesBuffer.forEach(function (event) {
                                    ...
                                    if (!event.candidate && self.iceGatheringState !== 'complete') {
                                        var complete = self.transceivers.every(function (transceiver) {
                                            return (
                                                transceiver.iceGatherer && transceiver.iceGatherer.state === 'completed'
                                            );
                                        });
                                        if (complete) {
                            ...
                            window.RTCPeerConnection.prototype.setRemoteDescription = function (description) {
                                ...
                                sections.forEach(function (mediaSection, sdpMLineIndex) {
                                    ...
                                    var isComplete =
                                        SDPUtils.matchPrefix(mediaSection, 'a=end-of-candidates').length > 0;
                                    var cands = SDPUtils.matchPrefix(mediaSection, 'a=candidate:')
                                        .map(function (cand) {
                                            return SDPUtils.parseCandidate(cand);
                                        })
